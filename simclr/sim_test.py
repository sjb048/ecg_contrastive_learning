...
if __name__ == "__main__":
    ...
    base_dir = '../data'
    wandb.init(project="smclr_umap_100", name="simclr_pretrain", config={
        "simclr_epochs": 100,
    ...
    ecg_tensor = torch.from_numpy(all_ecgs[:, None, :]).float()  # (N, 1, 1000)

    simclr_ds = AugmentationSimCLR(signals=ecg_tensor)
    simclr_loader = DataLoader(simclr_ds, batch_size=32, shuffle=True)

    # Define encoder + projection head
    encoder = ImprovedECGEncoder(in_channels=1, base_channels=64, embedding_dim=128)
    # simclr_model = SimCLR1D(encoder, emb_dim=128, proj_dim=128)
    ...
    trainer = SimCLRTrainer(simclr_model, optimizer, scheduler, args)
    trainer.train(simclr_loader)
    ...
    # → Train ECGClassifier using pretrained encoder
    ...
    # Phase 3: Evaluation
    wandb.init(project="ECG-SimCLR-final", name="simclr_evaluation")
    # → Evaluate on validation/test set (e.g., fold 6 or 10)
    ...
    test_metrics = evaluate_metrics(classifier, test_loader, device=device)
    evaluate_and_plot_confusion_matrix(classifier, test_loader, device=device)

    print("Test Metrics:", test_metrics)
    wandb.log(test_metrics)
    wandb.finish()
    # → Log confusion matrix & classification metrics
